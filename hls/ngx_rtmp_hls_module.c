...
#define NGX_RTMP_HLS_VAR_HEADER "#EXTM3U\n#EXT-X-VERSION:3\n"
    ...
    if (ngx_rtmp_hls_rename_file(ctx->var_playlist_bak.data,
                                 ctx->var_playlist.data) == NGX_FILE_ERROR)
    {
        ngx_log_error(NGX_LOG_ERR, s->connection->log, ngx_errno,
...
ngx_rtmp_hls_write_playlist(ngx_rtmp_session_t *s)
{
    ...
    if (ngx_rtmp_hls_rename_file(ctx->playlist_bak.data, ctx->playlist.data) == NGX_FILE_ERROR)
    {
        ngx_log_error(NGX_LOG_ERR, s->connection->log, ngx_errno,
                      "hls: rename failed: '%V'->'%V'",
                           ...
                           ngx_int_t discont)
{
    ...
    if (hacf->keys)
    {
        if (ctx->key_frags == 0)
        {
            ...
            strncpy(ctx->key, (unsigned char *)md5HexResult, 16);
        ...
        else
        {
            ...
            if (ngx_set_file_time(ctx->keyfile.data, 0, ngx_cached_time->sec) != NGX_OK)
            {
...
#define NGX_RTMP_MSEQ_LEN (sizeof(NGX_RTMP_MSEQ) - 1)
            ...
            if (ngx_memcmp(p, NGX_RTMP_MSEQ, NGX_RTMP_MSEQ_LEN) == 0)
            {
                ...
                ctx->frag = (uint64_t)strtod((const char *)&p[NGX_RTMP_MSEQ_LEN], NULL);
...
ngx_rtmp_hls_publish(ngx_rtmp_session_t *s, ngx_rtmp_publish_t *v)
{
    ...
    if (hacf->variant)
    {
        ...
        for (n = 0; n < hacf->variant->nelts; n++, var++)
        {
                           ...
                           var->suffix.len) == 0)
            {
                ...
                ctx->var_playlist.len = len - var->suffix.len + sizeof(".m3u8") - 1;
    ...
    *p = 0;
    ...
    if (hacf->keys)
    {
        len = hacf->key_path.len + 1 + ctx->name.len + 1 + NGX_INT64_LEN + sizeof(".key");
                             ...
                             ngx_int_t boundary, ngx_uint_t flush_rate)
{
    ...
    if (ctx->opened && b && b->last > b->pos &&
        ctx->aframe_pts + (uint64_t)hacf->max_audio_delay * 90 / flush_rate < ts)
    {
        ngx_rtmp_hls_flush_audio(s);
    }
}
